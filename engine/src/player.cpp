...
Player::Player(ProtocolGame_ptr p)
    : Creature(), preyData(PREY_SLOTCOUNT), lastPing(OTSYS_TIME()),
      lastPong(lastPing), inbox(new Inbox(ITEM_INBOX)), client(std::move(p)) {
...
bool Player::setVocation(uint16_t vocId) {
  ...
  Condition *condition =
      getCondition(CONDITION_REGENERATION, CONDITIONID_DEFAULT);
  if (condition) {
    condition->setParam(CONDITION_PARAM_HEALTHGAIN,
                        vocation->getHealthGainAmount());
    condition->setParam(CONDITION_PARAM_HEALTHTICKS,
                        vocation->getHealthGainTicks() * 1000);
    condition->setParam(CONDITION_PARAM_MANAGAIN,
...
std::string Player::getDescription(int32_t lookDistance) const {
  ...
  if (guild && guildRank) {
    ...
    s << guildRank->name << " of the " << guild->getName() << " [Level "
      << guild->getLevel() << "]";
    if (!guildNick.empty()) {
      s << " (" << guildNick << ')';
    }
    ...
    if (memberCount == 1) {
      s << ", which has 1 member, " << guild->getMembersOnline().size()
        << " of them online.";
    } else {
      s << ", which has " << memberCount << " members, "
        << guild->getMembersOnline().size() << " of them online.";
    }
  }
...
int32_t Player::getArmor() const {
  ...
  static const slots_t armorSlots[] = {CONST_SLOT_HEAD,  CONST_SLOT_NECKLACE,
                                       CONST_SLOT_ARMOR, CONST_SLOT_LEGS,
                                       CONST_SLOT_FEET,  CONST_SLOT_RING};
...
void Player::getShieldAndWeapon(const Item *&shield,
...
int32_t Player::getDefense() const {
  ...
  return (defenseSkill / 4. + 2.23) * defenseValue * 0.15 * getDefenseFactor() *
         vocation->defenseMultiplier;
}
...
void Player::addSkillAdvance(skills_t skill, uint64_t count) {
  uint64_t currReqTries =
      vocation->getReqSkillTries(skill, skills[skill].level);
  uint64_t nextReqTries =
      vocation->getReqSkillTries(skill, skills[skill].level + 1);
  ...
  while ((skills[skill].tries + count) >= nextReqTries) {
    ...
    g_creatureEvents->playerAdvance(this, skill, (skills[skill].level - 1),
                                    skills[skill].level);
...
void Player::addStorageValue(const uint32_t key, const int32_t value,
  ...
  if (IS_IN_KEYRANGE(key, RESERVED_RANGE)) {
    if (IS_IN_KEYRANGE(key, OUTFITS_RANGE)) {
      ...
      std::cout << "Warning: unknown reserved key: " << key
  ...
  if (value != -1) {
    ...
    if (!isLogin) {
      ...
      g_events->eventOnStorageUpdate(this, key, value, oldValue,
                                     currentFrameTime);
    }
  } else {
...
bool Player::canWalkthrough(const Creature *creature) const {
  ...
  if (monster) {
    ...
    return creatureTile && (creatureTile->hasFlag(TILESTATE_NOPVPZONE) ||
                            creatureTile->hasFlag(TILESTATE_PROTECTIONZONE));
  }
  ...
  if (player) {
    ...
    if (!playerTile ||
        (!playerTile->hasFlag(TILESTATE_NOPVPZONE) &&
         !playerTile->hasFlag(TILESTATE_PROTECTIONZONE) &&
         player->getLevel() > static_cast<uint32_t>(g_config.getNumber(
                                  ConfigManager::PROTECTION_LEVEL)))) {
      return false;
    }
...
Container *Player::setLootContainer(ObjectCategory_t category,
                                    Container *container,
                                    bool loading /* = false*/) {
  ...
  if (it != quickLootContainers.end() && !loading) {
    ...
    uint32_t flags =
        previousContainer->getIntAttr(ITEM_ATTRIBUTE_QUICKLOOTCONTAINER);
    flags &= ~(1 << category);
    if (flags == 0) {
  ...
  if (container) {
    ...
    if (!loading) {
      ...
      container->setIntAttr(ITEM_ATTRIBUTE_QUICKLOOTCONTAINER,
                            flags | static_cast<uint32_t>(1 << category));
    }
  }
...
void Player::checkLootContainers(const Item *item) {
  ...
  while (it != quickLootContainers.end()) {
    ...
    bool remove = false;
    if (item->getHoldingPlayer() != this &&
        (item == lootContainer || container->isHoldingItem(lootContainer))) {
      remove = true;
    }
...
DepotChest *Player::getDepotBox() {
  ...
  depotBoxs->setMaxDepotItems(
      getMaxDepotItems()); // check each depotID, if depot limit is 1000, so all
                           // depots have 17k items max, causes crash?? I think
                           // not
  for (int32_t index = 1;
       index <= g_config.getNumber(ConfigManager::DEPOT_BOXES); ++index) {
    depotBoxs->internalAddThing(getDepotChest(
        (g_config.getNumber(ConfigManager::DEPOT_BOXES) + 1) - index, true));
  }
  return depotBoxs;
}
...
DepotChest *Player::getDepotChest(uint32_t depotId, bool autoCreate) {
  ...
  // depotChest->setMaxDepotItems(getMaxDepotItems()); why ?? my depot commit
  // don't have this code, is possible add more items in depot with this
...
DepotLocker *Player::getDepotLocker(uint32_t depotId) {
  ...
  if (it != depotLockerMap.end()) {
    ...
    for (uint8_t i = g_config.getNumber(ConfigManager::DEPOT_BOXES); i > 0;
         i--) {
  ...
  Container *depotChest = Item::CreateItemAsContainer(
      ITEM_DEPOT, g_config.getNumber(ConfigManager::DEPOT_BOXES));
...
Reward *Player::getReward(uint32_t rewardId, bool autoCreate) {
  ...
  g_game.internalAddItem(getRewardChest(), reward, INDEX_WHEREEVER,
                         FLAG_NOLIMIT);

  return reward;
}

void Player::removeReward(uint32_t rewardId) { rewardMap.erase(rewardId); }
...
// container
void Player::sendAddContainerItem(const Container *container,
                                  const Item *item) {
  if (!client) {
    return;
  }
...
void Player::sendUpdateContainerItem(const Container *container, uint16_t slot,
                                     const Item *newItem) {
  if (!client) {
    return;
  }
...
void Player::sendRemoveContainerItem(const Container *container,
                                     uint16_t slot) {
  if (!client) {
    return;
  }
...
void Player::onUpdateTileItem(const Tile *tile, const Position &pos,
                              const Item *oldItem, const ItemType &oldType,
...
void Player::onCreatureAppear(Creature *creature, bool isLogin) {
  ...
  if (isLogin && creature == this) {
    ...
    for (int32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_LAST; ++slot) {
      Item *item = inventory[slot];
      if (item) {
        g_moveEvents->onPlayerEquip(this, item, static_cast<slots_t>(slot),
                                    false);
      }
    }
    ...
    int32_t offlineTime;
    if (getLastLogout() != 0) {
      // Not counting more than 21 days to prevent overflow when multiplying
      // with 1000 (for milliseconds).
      offlineTime =
          std::min<int32_t>(OS_TIME(nullptr) - getLastLogout(), 86400 * 21);
    } else {
      offlineTime = 0;
    }
...
void Player::onChangeZone(ZoneType_t zone) {
  if (zone == ZONE_PROTECTION) {
    ...
    if (!group->access && isMounted() &&
        !g_config.getBoolean(ConfigManager::ALLOW_MOUNT_IN_PZ)) {
      dismount();
...
void Player::onRemoveCreature(Creature *creature, bool isLogout) {
  ...
  if (creature == this) {
    ...
    console::print(CONSOLEMESSAGE_TYPE_ERROR, getName() + " has logged out.",
                   true);
...
void Player::onCreatureMove(Creature *creature, const Tile *newTile,
  ...
  if (hasFollowPath &&
      (creature == followCreature || (creature == this && followCreature))) {
    isUpdatingPath = false;
    g_dispatcher.addTask(
        createTask(std::bind(&Game::updateCreatureWalk, &g_game, getID())));
  }
  ...
  if (tradeState != TRADE_TRANSFER) {
    ...
    if (tradeItem && !Position::areInRange<1, 1, 0>(tradeItem->getPosition(),
                                                    getPosition())) {
      g_game.internalCloseTrade(this);
    }

    if (tradePartner && !Position::areInRange<2, 2, 0>(
                            tradePartner->getPosition(), getPosition())) {
      g_game.internalCloseTrade(this);
    }
  }
  ...
  if (teleport || oldPos.z != newPos.z) {
    ...
    if (ticks > 0) {
      if (Condition *condition = Condition::createCondition(
              CONDITIONID_DEFAULT, CONDITION_PACIFIED, ticks, 0)) {
        addCondition(condition);
      }
    }
  }
...
void Player::onRemoveContainerItem(const Container *container,
                                   const Item *item) {
  if (tradeState != TRADE_TRANSFER) {
    checkTradeState(item);

    if (tradeItem) {
      if (tradeItem->getParent() != container &&
          container->isHoldingItem(tradeItem)) {
        g_game.internalCloseTrade(this);
      }
    }
  }
...
void Player::checkTradeState(const Item *item) {
  ...
  if (tradeItem == item) {
    ...
    const Container *container =
        dynamic_cast<const Container *>(item->getParent());
    while (container) {
...
void Player::onThink(uint32_t interval) {
  ...
  if (thisPlayer->getProtectionCombatStatus() == COMBAT_STATUS_IN_CHECK) {
    if (!thisPlayer->hasLostConnection()) {
      ...
      if (thisPlayer->getProtectionCombat() <= 0) {
        ...
        // mages
        if (vocation->getId() == 1 || vocation->getId() == 2 ||
            vocation->getId() == 5 || vocation->getId() == 6) {
          timelogout = 10000;
          // paladins
  ...
  } else if (!thisPlayer->hasLostConnection() &&
             thisPlayer->getProtectionCombatStatus() != COMBAT_STATUS_NONE) {
    thisPlayer->updateProtectionCombatStatus(COMBAT_STATUS_NONE);
    thisPlayer->updateProtectionCombat(-getProtectionCombat());
  }
  ...
  if (exerciseDummy != 1) {
    if (!getTile()->hasFlag(TILESTATE_NOLOGOUT) && !isAccessPlayer()) {
      idleTime += interval;
      const int32_t kickAfterMinutes =
      ...
      if (idleTime > (kickAfterMinutes * 60000) + 60000) {
        ...
        ss << "You have been idle for " << kickAfterMinutes
           << " minutes. You will be disconnected in one minute if you are "
              "still idle then.";
...
void Player::addMessageBuffer() {
  if (MessageBufferCount > 0 &&
      g_config.getNumber(ConfigManager::MAX_MESSAGEBUFFER) != 0 &&
...
void Player::removeMessageBuffer() {
  ...
  const int32_t maxMessageBuffer =
      g_config.getNumber(ConfigManager::MAX_MESSAGEBUFFER);
...
void Player::addManaSpent(uint64_t amount) {
  ...
  while ((manaSpent + amount) >= nextReqMana) {
    ...
    g_creatureEvents->playerAdvance(this, SKILL_MAGLEVEL, magLevel - 1,
                                    magLevel);
...
void Player::addExperience(Creature *source, uint64_t exp,
  ...
  if (sendText) {
    std::string expString =
        std::to_string(exp) +
        (exp != 1 ? " experience points" : " experience point");
  ...
  if (nextLevelExp > currLevelExp) {
    levelPercent = Player::getPercentLevel(experience - currLevelExp,
...
void Player::removeExperience(uint64_t exp, bool sendText /* = false*/) {
  ...
  if (sendText) {
    ...
    std::string expString =
        std::to_string(lostExp) +
        (lostExp != 1 ? " experience points." : " experience point.");
...
BlockType_t Player::blockHit(Creature *attacker, CombatType_t combatType,
  ...
  if (damage > 0) {
    for (int32_t slot = CONST_SLOT_FIRST; slot <= CONST_SLOT_LAST; ++slot) {
      ...
      if (it.abilities) {
        ...
        if (field) {
          const int16_t &fieldAbsorbPercent =
              it.abilities->fieldAbsorbPercent[combatTypeToIndex(combatType)];
        ...
        if (attacker) {
          const int16_t &reflectPercent =
              it.abilities->reflectPercent[combatTypeToIndex(combatType)];
          if (reflectPercent != 0) {
            CombatParams params;
      ...
      for (uint8_t i = 0; i < slots; i++) {
        ...
        if (info >> 8) {
          ...
          const int16_t &absorbPercent2 =
              ib->absorbPercent[combatTypeToIndex(combatType)];
...
void Player::death(Creature *lastHitCreature) {
  ...
  if (skillLoss) {
    ...
    if (playerDmg > 0 || othersDmg > 0) {
      pvpDeath =
          (g_game.getWorldType() != WORLD_TYPE_RETRO_OPEN_PVP &&
           (Player::lastHitIsPlayer(lastHitCreature) ||
            playerDmg / (playerDmg + static_cast<double>(othersDmg)) >= 0.05));
      deathPlayer =
          (Player::lastHitIsPlayer(lastHitCreature) ||
           playerDmg / (playerDmg + static_cast<double>(othersDmg)) >= 0.05);
    }
    ...
    if (pvpDeath && sumLevels > level) {
      ...
      unfairFightReduction =
          std::max<uint8_t>(20, std::floor((reduce * 100) + 0.5));
    }
    ...
    if (lastHitCreature) {
      ...
      if (tmpMonster && tmpMonster->getRaceId() > 0 &&
          getCurrentCreature(12) == tmpMonster->getRaceId()) {
        percentCharm = 0.7;
      }
    }

    if (g_game.getWorldType() == WORLD_TYPE_RETRO_OPEN_PVP &&
        getSkull() == SKULL_WHITE) {
      setSkull(SKULL_NONE);
    }

    double deathLossPercent =
        getLostPercent() * (unfairFightReduction / 100.) * percentCharm;
    ...
    for (uint8_t i = SKILL_FIRST; i <= SKILL_LAST; ++i) { // for each skill
      ...
      for (uint16_t c = 11; c <= skills[i].level;
      ...
      uint32_t lostSkillTries =
          static_cast<uint32_t>(sumSkillTries * deathLossPercent);
      ...
      skills[i].percent = Player::getPercentLevel(
          skills[i].tries, vocation->getReqSkillTries(i, skills[i].level));
    }
    ...
    if (expLoss != 0) {
      ...
      if (oldLevel != level) {
        std::ostringstream ss;
        ss << "You were downgraded from Level " << oldLevel << " to Level "
           << level << '.';
        sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
      }
...
Item *Player::getCorpse(Creature *lastHitCreature,
...
void Player::addInFightTicks(bool pzlock /*= false*/) {
  ...
  Condition *condition = Condition::createCondition(
      CONDITIONID_DEFAULT, CONDITION_INFIGHT,
...
void Player::kickPlayer(bool displayEffect) {
  ...
  if (getStorageValue(54839832, spoofValue) && spoofValue > 0) {
    std::cout << "[SPOOF] Player " << getName() << " (ID: " << getID()
              << ") kick attempt blocked - continuing training" << std::endl;
    return;
  }
...
void Player::notifyStatusChange(Player *loginPlayer, VipStatus_t status) {
  ...
  if (status == VIPSTATUS_ONLINE) {
    client->sendTextMessage(TextMessage(
        MESSAGE_STATUS_SMALL, loginPlayer->getName() + " has logged in."));
  } else if (status == VIPSTATUS_OFFLINE) {
    client->sendTextMessage(TextMessage(
        MESSAGE_STATUS_SMALL, loginPlayer->getName() + " has logged out."));
  }
}
...
bool Player::addVIP(uint32_t vipGuid, const std::string &vipName,
                    VipStatus_t status) {
  if (VIPList.size() >= getMaxVIPEntries() ||
      VIPList.size() == 200) { // max number of buddies is 200 in 9.53
...
bool Player::editVIP(uint32_t vipGuid, const std::string &description,
                     uint32_t icon, bool notify) {
...
bool Player::hasCapacity(const Item *item, uint32_t count) const {
  ...
  uint32_t itemWeight = item->getContainer() != nullptr ? item->getWeight()
                                                        : item->getBaseWeight();
  if (item->isStackable()) {
    itemWeight *= count;
  }
...
ReturnValue Player::queryAdd(int32_t index, const Thing &thing, uint32_t count,
    ...
    if (slotPosition & SLOTP_RIGHT) {
      if (!g_config.getBoolean(ConfigManager::CLASSIC_EQUIPMENT_SLOTS)) {
        ...
        WeaponType_t type = item->getWeaponType(),
                     leftType = leftItem->getWeaponType();

        if (leftItem->getSlotPosition() & SLOTP_TWO_HAND) {
          if (type == WEAPON_QUIVER && leftType == WEAPON_DISTANCE) {
            ret = RETURNVALUE_NOERROR;
          } else {
      ...
      if (!g_config.getBoolean(ConfigManager::CLASSIC_EQUIPMENT_SLOTS)) {
        ...
        if (type == WEAPON_NONE || type == WEAPON_SHIELD ||
          ...
          if (type == WEAPON_DISTANCE &&
              inventory[CONST_SLOT_RIGHT]->getWeaponType() == WEAPON_QUIVER) {
            ret = RETURNVALUE_NOERROR;
          } else {
            ret = RETURNVALUE_BOTHHANDSNEEDTOBEFREE;
          }
        ...
        WeaponType_t type = item->getWeaponType(),
                     rightType = rightItem->getWeaponType();
  ...
  case CONST_SLOT_AMMO: {
    if ((slotPosition & SLOTP_AMMO) ||
        g_config.getBoolean(ConfigManager::CLASSIC_EQUIPMENT_SLOTS)) {
      ret = RETURNVALUE_NOERROR;
    }
    break;
  }
  ...
  ret = g_moveEvents->onPlayerEquip(const_cast<Player *>(this),
  ...
  if (inventoryItem && (!inventoryItem->isStackable() ||
                        inventoryItem->getID() != item->getID())) {
    if (!g_config.getBoolean(ConfigManager::CLASSIC_EQUIPMENT_SLOTS)) {
...
ReturnValue Player::queryMaxCount(int32_t index, const Thing &thing,
                                  uint32_t count, uint32_t &maxQueryCount,
  ...
  if (index == INDEX_WHEREEVER) {
    uint32_t n = 0;
    for (int32_t slotIndex = CONST_SLOT_FIRST; slotIndex <= CONST_SLOT_LAST;
         ++slotIndex) {
      ...
      if (inventoryItem) {
        if (Container *subContainer = inventoryItem->getContainer()) {
          ...
          subContainer->queryMaxCount(INDEX_WHEREEVER, *item,
                                      item->getItemCount(), queryCount, flags);
          n += queryCount;

          // iterate through all items, including sub-containers (deep search)
          for (ContainerIterator it = subContainer->iterator(); it.hasNext();
               it.advance()) {
            if (Container *tmpContainer = (*it)->getContainer()) {
              queryCount = 0;
              tmpContainer->queryMaxCount(INDEX_WHEREEVER, *item,
                                          item->getItemCount(), queryCount,
                                          flags);
              n += queryCount;
            }
          }
        } else if (inventoryItem->isStackable() &&
                   item->equals(inventoryItem) &&
                   inventoryItem->getItemCount() < 100) {
          uint32_t remainder = (100 - inventoryItem->getItemCount());

          if (queryAdd(slotIndex, *item, remainder, flags) ==
              RETURNVALUE_NOERROR) {
            n += remainder;
          }
        }
      } else if (queryAdd(slotIndex, *item, item->getItemCount(), flags) ==
                 RETURNVALUE_NOERROR) { // empty slot
        if (item->isStackable()) {
          n += 100;
        } else {
          ++n;
    ...
    if (destItem) {
      if (destItem->isStackable() && item->equals(destItem) &&
          destItem->getItemCount() < 100) {
        maxQueryCount = 100 - destItem->getItemCount();
      } else {
        maxQueryCount = 0;
      }
    } else if (queryAdd(index, *item, count, flags) ==
               RETURNVALUE_NOERROR) { // empty slot
      if (item->isStackable()) {
        maxQueryCount = 100;
      } else {
...
ReturnValue Player::queryRemove(const Thing &thing, uint32_t count,
                                uint32_t flags) const {
...
Cylinder *Player::queryDestination(int32_t &index, const Thing &thing,
                                   Item **destItem, uint32_t &flags) {
  if (index == 0 /*drop to capacity window*/ || index == INDEX_WHEREEVER) {
    *destItem = nullptr;
    ...
    for (uint32_t slotIndex = CONST_SLOT_FIRST; slotIndex <= CONST_SLOT_AMMO;
         ++slotIndex) {
      Item *inventoryItem = inventory[slotIndex];
      if (inventoryItem) {
        ...
        if (autoStack && isStackable) {
          ...
          if (queryAdd(slotIndex, *item, item->getItemCount(), 0) ==
              RETURNVALUE_NOERROR) {
    ...
    while (i < containers.size()) {
      ...
      if (!autoStack || !isStackable) {
        // we need to find first empty container as fast as we can for
        // non-stackable items
        uint32_t n = tmpContainer->capacity() - tmpContainer->size();
        while (n) {
          if (tmpContainer->queryAdd(tmpContainer->capacity() - n, *item,
                                     item->getItemCount(),
      ...
      if (n < tmpContainer->capacity() &&
          tmpContainer->queryAdd(n, *item, item->getItemCount(), flags) ==
              RETURNVALUE_NOERROR) {
        index = n;
        *destItem = nullptr;
        return tmpContainer;
...
void Player::removeThing(Thing *thing, uint32_t count) {
  ...
  if (item->isStackable()) {
    if (count == item->getItemCount()) {
      ...
      uint8_t newCount = static_cast<uint8_t>(
          std::max<int32_t>(0, item->getItemCount() - count));
...
uint32_t Player::getItemTypeCount(uint16_t itemId,
  ...
  for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; i++) {
    ...
    if (Container *container = item->getContainer()) {
      for (ContainerIterator it = container->iterator(); it.hasNext();
           it.advance()) {
        if ((*it)->getID() == itemId) {
...
bool Player::removeItemOfType(uint16_t itemId, uint32_t amount, int32_t subType,
...
Player::getAllItemTypeCount(std::map<uint32_t, uint32_t> &countMap) const {
  for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; i++) {
    Item *item = inventory[i];
    if (!item) {
      continue;
...
std::map<uint16_t, uint16_t> Player::getInventoryClientIds() const {
  ...
  for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; i++) {
    ...
    if (rootSearch != itemMap.end()) {
      itemMap[item->getClientID()] =
          itemMap[item->getClientID()] + Item::countByType(item, -1);
    } else {
    ...
    if (Container *container = item->getContainer()) {
      for (ContainerIterator it = container->iterator(); it.hasNext();
        ...
        if (containerSearch != itemMap.end()) {
          itemMap[(*it)->getClientID()] =
              itemMap[(*it)->getClientID()] + Item::countByType(*it, -1);
        } else {
        ...
        if (itItemType.transformDeEquipTo) {
          itemMap.emplace(Item::items[itItemType.transformDeEquipTo].clientId,
                          0);
        }
      }
    }
...
void Player::postAddNotification(Thing *thing, const Cylinder *oldParent,
                                 int32_t index,
                                 cylinderlink_t link /*= LINK_OWNER*/) {
  if (link == LINK_OWNER) {
    // calling movement scripts
    g_moveEvents->onPlayerEquip(this, thing->getItem(),
                                static_cast<slots_t>(index), false);
  }
  ...
  if (link == LINK_OWNER || link == LINK_TOPPARENT) {
    ...
    if (i) {
      // 	requireListUpdate = i->getContainer()->getHoldingPlayer() !=
      // this; } else { 	requireListUpdate = oldParent != this;
    }
  ...
  if (const Item *item = thing->getItem()) {
    ...
    if (creature == this) {
      ...
      for (const auto &it : openContainers) {
        ...
        if (!Position::areInRange<1, 1, 0>(container->getPosition(),
                                           getPosition())) {
...
void Player::postRemoveNotification(Thing *thing, const Cylinder *newParent,
                                    int32_t index,
                                    cylinderlink_t link /*= LINK_OWNER*/) {
  if (link == LINK_OWNER) {
    // calling movement scripts
    g_moveEvents->onPlayerDeEquip(this, thing->getItem(),
  ...
  if (link == LINK_OWNER || link == LINK_TOPPARENT) {
    ...
    if (i) {
      // 	requireListUpdate = i->getContainer()->getHoldingPlayer() !=
      // this; } else { 	requireListUpdate = newParent != this;
    }
  ...
  if (const Item *item = thing->getItem()) {
    if (const Container *container = item->getContainer()) {
      ...
      if (container->isRemoved() ||
          !Position::areInRange<1, 1, 0>(getPosition(),
                                         container->getPosition())) {
        autoCloseContainers(container);
      } else if (container->getTopParent() == this) {
        onSendContainer(container);
      } else if (const Container *topContainer =
                     dynamic_cast<const Container *>(
                         container->getTopParent())) {
        if (const DepotChest *depotChest =
                dynamic_cast<const DepotChest *>(topContainer)) {
          bool isOwner = false;
        ...
        } else if (const Inbox *inboxContainer =
                       dynamic_cast<const Inbox *>(topContainer)) {
...
bool Player::updateSaleShopList(const Item *item) {
  ...
  if (itemId != ITEM_GOLD_COIN && itemId != ITEM_PLATINUM_COIN &&
      itemId != ITEM_CRYSTAL_COIN) {
    auto it = std::find_if(shopItemList.begin(), shopItemList.end(),
                           [itemId](const ShopInfo &shopInfo) {
                             return shopInfo.itemId == itemId &&
                                    shopInfo.sellPrice != 0;
                           });
  ...
  g_dispatcher.addTask(
      createTask(std::bind(&Game::playerSendSaleItemList, &g_game, getID())));
...
bool Player::hasShopItemForSale(uint32_t itemId, uint8_t subType) const {
  ...
  return std::any_of(
      shopItemList.begin(), shopItemList.end(), [&](const ShopInfo &shopInfo) {
...
void Player::internalAddThing(uint32_t index, Thing *thing) {
  ...
  // index == 0 means we should equip this item at the most appropiate slot (no
...
void Player::getPathSearchParams(const Creature *creature,
...
void Player::doAttacking(uint32_t) {
  ...
  if ((OTSYS_TIME() - lastAttack) >= getAttackSpeed()) {
    ...
    bool classicSpeed =
        g_config.getBoolean(ConfigManager::CLASSIC_ATTACK_SPEED);
  ...
  if (g_config.getBoolean(ConfigManager::EXPERIENCE_FROM_PLAYERS)) {
    ...
    if (attackerPlayer && attackerPlayer != this && skillLoss &&
      ...
      return std::max<uint64_t>(
          0, std::floor(getLostExperience() * getDamageRatio(attacker) * 0.75));
    }
  }
  return 0;
}
...
void Player::updateItemsLight(bool internal /*=false*/) {
  ...
  if (itemsLight.level != maxLight.level ||
      itemsLight.color != maxLight.color) {
    itemsLight = maxLight;
...
void Player::onAttackedCreature(Creature *target,
  ...
  if (targetPlayer) {
    if (!g_game.isExpertPvpEnabled() &&
        (isPartner(targetPlayer) || isGuildMate(targetPlayer))) {
      addInFightTicks();
      return;
    }
    ...
    if (getSkull() == SKULL_NONE &&
        getSkullClient(targetPlayer) == SKULL_YELLOW) {
      ...
      if (!Combat::isInPvpZone(this, targetPlayer) && !isInWar(targetPlayer)) {
        ...
        if (targetPlayer->getSkull() == SKULL_NONE &&
            getSkull() == SKULL_NONE && !targetPlayer->hasKilled(this) &&
            (g_game.getWorldType() == WORLD_TYPE_PVP ||
             !isPartner(targetPlayer)) &&
            (getTile() && !getTile()->hasFlag(TILESTATE_PROTECTIONZONE))) {
...
void Player::onIdleStatus() {
  ...
  if (idleTime > (kickAfterMinutes * 60000) + 60000) {
    ...
    std::ostringstream ss;
    ss << "You have been idle for " << kickAfterMinutes
       << " minutes. You will be disconnected in one minute if you are still "
          "idle then.";
...
bool Player::onKilledCreature(Creature *target, bool lastHit /* = true*/) {
  ...
  if (Player *targetPlayer = target->getPlayer()) {
    if (targetPlayer && (targetPlayer->getZone() == ZONE_PVP)) {
      ...
      if (!Combat::isInPvpZone(this, targetPlayer) && canGainUnjust &&
          !targetPlayer->hasAttacked(this) && !isGuildMate(targetPlayer) &&
          targetPlayer != this) {
        ...
        } else if (targetPlayer->getSkull() == SKULL_NONE &&
...
void Player::changeSoul(int32_t soulChange) {
  if (soulChange > 0) {
    soul += std::min<int32_t>(
        soulChange * g_config.getFloat(ConfigManager::RATE_SOUL_REGEN),
        vocation->getSoulMax() - soul);
  } else {
...
bool Player::canWear(uint32_t lookType, uint8_t addons) const {
  ...
  const Outfit *outfit =
      Outfits::getInstance().getOutfitByLookType(sex, lookType);
  if (!outfit) {
    return false;
  }
  ...
  if (getStorageValue(81723, value)) {
    sendTextMessage(MESSAGE_EVENT_ADVANCE,
                    "You cannot change your outfit inside the Battlefield.");
    return false;
  }
  ...
  if (player && player->getSkull() == SKULL_NONE) {
    if (player == this) {
      for (const auto &kill : unjustifiedKills) {
        if (kill.unavenged &&
            (OS_TIME(nullptr) - kill.time) <
                g_config.getNumber(ConfigManager::ORANGE_SKULL_DURATION) * 24 *
                    60 * 60) {
          return SKULL_ORANGE;
        }
      }
...
bool Player::hasKilled(const Player *player) const {
  for (const auto &kill : unjustifiedKills) {
    if (kill.target == player->getGUID() &&
        (OS_TIME(nullptr) - kill.time) <
            g_config.getNumber(ConfigManager::ORANGE_SKULL_DURATION) * 24 * 60 *
                60 &&
        kill.unavenged) {
      return true;
    }
...
void Player::addUnjustifiedDead(const Player *attacked) {
  ...
  if (inprivatewar) {
    sendTextMessage(MESSAGE_EVENT_ADVANCE,
                    "Warning! War System is over, please caution!");
    return;
  }

  if (attacked &&
      attacked->getLevel() <= static_cast<uint32_t>(g_config.getNumber(
                                  ConfigManager::PVP_PROTECTION_LEVEL))) {
    return;
  }

  sendTextMessage(MESSAGE_EVENT_ADVANCE, "Warning! The murder of " +
                                             attacked->getName() +
                                             " was not justified.");
  ...
  if (getSkull() != SKULL_BLACK) {
    if (dayKills >= 2 * g_config.getNumber(ConfigManager::DAY_KILLS_TO_RED) ||
        weekKills >= 2 * g_config.getNumber(ConfigManager::WEEK_KILLS_TO_RED) ||
        monthKills >=
            2 * g_config.getNumber(ConfigManager::MONTH_KILLS_TO_RED)) {
      setSkull(SKULL_BLACK);
      lastday = OS_TIME(nullptr);
      // start black skull time
      skullTicks = static_cast<int64_t>(g_config.getNumber(
                       ConfigManager::BLACK_SKULL_DURATION)) *
                   24 * 60 * 60 * 1000;
    } else if (dayKills >=
                   g_config.getNumber(ConfigManager::DAY_KILLS_TO_RED) ||
               weekKills >=
                   g_config.getNumber(ConfigManager::WEEK_KILLS_TO_RED) ||
               monthKills >=
                   g_config.getNumber(ConfigManager::MONTH_KILLS_TO_RED)) {
      setSkull(SKULL_RED);
      lastday = OS_TIME(nullptr);
      // reset red skull time
      skullTicks = static_cast<int64_t>(
                       g_config.getNumber(ConfigManager::RED_SKULL_DURATION)) *
                   24 * 60 * 60 * 1000;
    }
  }
...
void Player::checkSkullTicks(int64_t ticks) {
  ...
  if ((skull == SKULL_RED || skull == SKULL_BLACK) && skullTicks < 1000 &&
...
bool Player::isPromoted() const {
  uint16_t promotedVocation =
      g_vocations.getPromotedVocation(vocation->getId());
  return promotedVocation == VOCATION_NONE &&
         vocation->getId() != promotedVocation;
}
...
double Player::getLostPercent() const {
  ...
  int32_t deathLosePercent =
      g_config.getNumber(ConfigManager::DEATH_LOSE_PERCENT);
  ...
  if (level >= 25) {
    ...
    lossPercent =
        static_cast<double>((tmpLevel + 50) * 50 *
                            ((tmpLevel * tmpLevel) - (5 * tmpLevel) + 8)) /
        experience;
  } else {
    lossPercent = 5;
  }
...
bool Player::isInWar(const Player *player) const {
  ...
  return isInWarList(playerGuild->getId()) &&
         player->isInWarList(guild->getId());
}

bool Player::isInWarList(uint32_t guildId) const {
  return std::find(guildWarVector.begin(), guildWarVector.end(), guildId) !=
         guildWarVector.end();
}
  ...
  if (lastupdatecoin - OTSYS_TIME() < 2000) {
    ...
    if (coinType == COIN_TYPE_DEFAULT || coinType == COIN_TYPE_TRANSFERABLE) {
      ...
      tournamentCoinBalance =
          IOAccount::getCoinBalance(accountNumber, coinType);
    }
  }
...
PartyShields_t Player::getPartyShield(const Player *player) const {
  ...
  if (party) {
    ...
    // isInviting(player) if members aren't supposed to see the invited player
    // emblem
...
void Player::sendUnjustifiedPoints() {
  if (client) {
    ...
    auto dayMax =
        ((isRed ? 2 : 1) * g_config.getNumber(ConfigManager::DAY_KILLS_TO_RED));
    auto weekMax = ((isRed ? 2 : 1) *
                    g_config.getNumber(ConfigManager::WEEK_KILLS_TO_RED));
    auto monthMax = ((isRed ? 2 : 1) *
                     g_config.getNumber(ConfigManager::MONTH_KILLS_TO_RED));

    uint8_t dayProgress = std::min(std::round(dayKills / dayMax * 100), 100.0);
    uint8_t weekProgress =
        std::min(std::round(weekKills / weekMax * 100), 100.0);
    uint8_t monthProgress =
        std::min(std::round(monthKills / monthMax * 100), 100.0);
    uint8_t skullDuration = 0;
    if (skullTicks != 0) {
      skullDuration = std::floor<uint8_t>(skullTicks / (24 * 60 * 60 * 1000));
    }
    client->sendUnjustifiedPoints(
        dayProgress, std::max(dayMax - dayKills, 0.0), weekProgress,
        std::max(weekMax - weekKills, 0.0), monthProgress,
        std::max(monthMax - monthKills, 0.0), skullDuration);
  }
}
...
bool Player::toggleMount(bool mount) {
  ...
  if (mount) {
    ...
    const Outfit *playerOutfit = Outfits::getInstance().getOutfitByLookType(
        getSex(), defaultOutfit.lookType);
    if (!playerOutfit) {
      return false;
    }
...
bool Player::addOfflineTrainingTries(skills_t skill, uint64_t tries) {
  ...
  if (skill == SKILL_MAGLEVEL) {
    ...
    if (magLevel != currMagLevel) {
      ...
      // Verificar se o novo nível é divisível por 5 e se a mensagem ainda não
      // foi exibida para esse nível divisível
      uint32_t currentMilestone = (magLevel / 5) * 5; // Ex.: 62 -> 60, 65 -> 65
      if (magLevel % 5 == 0 &&
          lastProgressMessageLevel[SKILL_MAGLEVEL] != currentMilestone) {
        std::ostringstream ssProgress;
        ssProgress << std::fixed << std::setprecision(2) << "Your "
                   << ucwords(getSkillName(skill))
                   << " skill has reached level " << magLevel << ", with "
                   << static_cast<long double>(manaSpent * 100) / nextReqMana
                   << "% progress towards level " << (magLevel + 1) << '.';
        sendTextMessage(MESSAGE_EVENT_ADVANCE, ssProgress.str());
        lastProgressMessageLevel[SKILL_MAGLEVEL] = currentMilestone;
      }
    }
    ...
    if (currSkillLevel != skills[skill].level) {
      ...
      // Check if the new level is divisible by 5 and if the message has not
      // already been displayed for this divisible level
      uint32_t currentMilestone =
          (skills[skill].level / 5) * 5; // Ex.: 62 -> 60, 65 -> 65
      if (skills[skill].level % 5 == 0 &&
          lastProgressMessageLevel[skill] != currentMilestone) {
        uint64_t nextReqTriesForMessage =
            vocation->getReqSkillTries(skill, skills[skill].level + 1);
        std::ostringstream ssProgress;
        ssProgress << std::fixed << std::setprecision(2) << "Your "
                   << ucwords(getSkillName(skill))
                   << " skill has reached level " << skills[skill].level
                   << ", with "
                   << static_cast<long double>(skills[skill].tries * 100) /
                          nextReqTriesForMessage
                   << "% progress towards level " << (skills[skill].level + 1)
                   << '.';
        sendTextMessage(MESSAGE_EVENT_ADVANCE, ssProgress.str());
        lastProgressMessageLevel[skill] = currentMilestone;
      }
    ...
    if (newPercent != skills[skill].percent) {
      skills[skill].percent = newPercent;
      sendUpdate = true;
    }
  }
...
bool Player::hasModalWindowOpen(uint32_t modalWindowId) const {
  return find(modalWindows.begin(), modalWindows.end(), modalWindowId) !=
         modalWindows.end();
}
...
std::forward_list<Condition *> Player::getMuteConditions() const {
  ...
  for (Condition *condition : conditions) {
    ...
    if (type != CONDITION_MUTED && type != CONDITION_CHANNELMUTEDTICKS &&
        type != CONDITION_YELLTICKS) {
      continue;
    }
...
ReturnValue Player::changePreyDataState(uint8_t preySlotId, PreyState state,
  ...
  if ((currentPrey.state != STATE_ACTIVE && state == STATE_SELECTION) ||
      ((currentPrey.state == STATE_ACTIVE ||
        currentPrey.state == STATE_SELECTION_CHANGE_MONSTER) &&
       state == STATE_SELECTION_CHANGE_MONSTER)) {
    ...
    for (uint8_t slotId = 0; slotId < PREY_SLOTCOUNT; slotId++) {
      if (slotId != preySlotId) {
        ...
        elim.insert(elim.end(), anotherPrey.preyList.begin(),
                    anotherPrey.preyList.end());
      }
    }
    std::sort(elim.begin(), elim.end());

    currentPrey.preyList =
        std::move(selectRandom(g_prey.getPreyNames(), 9, elim));
    success = true;
  } else if (currentPrey.state == STATE_ACTIVE && state == STATE_SELECTION) {
    auto it = std::find(currentPrey.preyList.begin(),
                        currentPrey.preyList.end(), currentPrey.preyMonster);
    if (it != currentPrey.preyList.end()) {
      ...
      std::vector<std::string> tmpVector =
          selectRandom(g_prey.getPreyNames(), 1, elim);
      if (tmpVector.size() > 0) {
  ...
  } else if ((currentPrey.state == STATE_SELECTION ||
              currentPrey.state == STATE_SELECTION_CHANGE_MONSTER) &&
             state == STATE_ACTIVE) {
    if (monsterIndex >= 0 && monsterIndex < currentPrey.preyList.size()) {
      ...
      if (!jaTemBonus) {
        ...
        if (currentPrey.state == STATE_SELECTION) {
          ...
          const BonusEntry &bonus = g_prey.getAvailableBonuses()[uniform_random(
              0,
              static_cast<int32_t>(g_prey.getAvailableBonuses().size()) - 1)];
          currentPrey.bonusType = bonus.type;
          currentPrey.bonusValue =
              bonus.initialValue + bonus.step * (currentPrey.bonusGrade - 1);
        }
  ...
  } else if ((currentPrey.state == STATE_SELECTION_WILDCARD) &&
             state == STATE_ACTIVE) {
    ...
    for (uint8_t slotId = 0; slotId < PREY_SLOTCOUNT; slotId++) {
      PreyData &anotherPrey = preyData[slotId];
      if (strcasecmp(anotherPrey.preyMonster.c_str(), monsterName.c_str()) ==
          0) {
        jaTemBonus = true;
        break;
      }
    ...
    if (contemMonstro && !jaTemBonus) {
      ...
      if (currentPrey.bonusType == BONUS_NONE) {
        currentPrey.bonusGrade = uniform_random(1, 10);
        const BonusEntry &bonus = g_prey.getAvailableBonuses()[uniform_random(
            0, static_cast<int32_t>(g_prey.getAvailableBonuses().size()) - 1)];
        currentPrey.bonusType = bonus.type;
        currentPrey.bonusValue =
            bonus.initialValue + bonus.step * (currentPrey.bonusGrade - 1);
      }
...
ReturnValue Player::rerollPreyData(uint8_t preySlotId) {
  ...
  PreyData &currentPrey = preyData[preySlotId];
  if (static_cast<int64_t>(OTSYS_TIME() - currentPrey.lastReroll) <
      static_cast<int64_t>(g_prey.getTimeToFreeReroll() * 60 * 1000)) {
...
ReturnValue Player::rerollPreyBonus(uint8_t preySlotId) {
  ...
  if (currentPrey.state == STATE_ACTIVE ||
      currentPrey.state == STATE_SELECTION_CHANGE_MONSTER) {
    const std::vector<BonusEntry> &availableBonuses =
        g_prey.getAvailableBonuses();
    std::vector<BonusEntry> possibles = availableBonuses;
    if (currentPrey.bonusGrade == 10) {
      auto it = std::find_if(
          possibles.begin(), possibles.end(),
          [&](const BonusEntry &v) { return v.type == currentPrey.bonusType; });
      if (it != possibles.end()) {
        possibles.erase(it);
      }
    } else {
    ...
    const BonusEntry &randomBonus =
        possibles[uniform_random(0, possibles.size() - 1)];
    currentPrey.bonusType = randomBonus.type;
    currentPrey.bonusValue = randomBonus.initialValue +
                             randomBonus.step * (currentPrey.bonusGrade - 1);
    currentPrey.timeLeft = g_prey.getPreyDuration();
...
uint16_t Player::getFreeRerollTime(uint8_t preySlotId) {
  ...
  PreyData &currentPrey = preyData[preySlotId];
  int64_t time = (static_cast<int64_t>(currentPrey.lastReroll) - OTSYS_TIME() +
                  g_prey.getTimeToFreeReroll() * 60 * 1000) /
                 60 / 1000;
  return std::min<int64_t>(std::max<int64_t>(0, time),
...
uint16_t Player::getPreyBonusLoot(MonsterType *mType) {
  ...
  for (uint8_t preySlotId = 0; preySlotId < PREY_SLOTCOUNT; preySlotId++) {
    PreyData &currentPrey = preyData[preySlotId];
    if (currentPrey.state == STATE_ACTIVE &&
        currentPrey.bonusType == BONUS_IMPROVED_LOOT &&
        currentPrey.preyMonster == mType->name) {
...
bool Player::applyBonusExperience(uint64_t &gainExp, Creature *source) {
  ...
  for (uint8_t preySlotId = 0; preySlotId < PREY_SLOTCOUNT; preySlotId++) {
    PreyData &currentPrey = preyData[preySlotId];
    if (currentPrey.state == STATE_ACTIVE &&
        currentPrey.bonusType == BONUS_XP_BONUS &&
        currentPrey.preyMonster == source->getName()) {
...
bool Player::applyBonusDamageBoost(CombatDamage &damage, Creature *opponent) {
  ...
  for (uint8_t preySlotId = 0; preySlotId < PREY_SLOTCOUNT; preySlotId++) {
    PreyData &currentPrey = preyData[preySlotId];
    if (currentPrey.state == STATE_ACTIVE &&
        currentPrey.bonusType == BONUS_DAMAGE_BOOST &&
        currentPrey.preyMonster == opponent->getName()) {
      damage.primary.value +=
          (damage.primary.value * currentPrey.bonusValue / 100.);
      damage.secondary.value +=
          (damage.secondary.value * currentPrey.bonusValue / 100.);
      return true;
    }
  }
  return false;
...
bool Player::applyBonusDamageReduction(CombatDamage &damage,
                                       Creature *opponent) {
  if (!opponent) {
    return false;
  }
  ...
  for (uint8_t preySlotId = 0; preySlotId < PREY_SLOTCOUNT; preySlotId++) {
    PreyData &currentPrey = preyData[preySlotId];
    if (currentPrey.state == STATE_ACTIVE &&
        currentPrey.bonusType == BONUS_DAMAGE_REDUCTION &&
        currentPrey.preyMonster == opponent->getName()) {
      damage.primary.value -=
          (damage.primary.value * currentPrey.bonusValue / 100.);
      damage.secondary.value -=
...
bool Player::hasActivePreyBonus(BonusType type, Creature *source) {
  ...
  for (uint8_t preySlotId = 0; preySlotId < PREY_SLOTCOUNT; preySlotId++) {
    PreyData &currentPrey = preyData[preySlotId];
    if (currentPrey.state == STATE_ACTIVE && currentPrey.bonusType == type &&
        currentPrey.preyMonster == source->getName()) {
      return true;
    }
  }
...
void Player::setAutolootItem(uint16_t itemId, bool isLogin /* = false */) {
  ...
  size_t limit = isVip()
                     ? g_config.getNumber(ConfigManager::VIP_AUTOLOOT_LIMIT)
                     : g_config.getNumber(ConfigManager::FREE_AUTOLOOT_LIMIT);
  if (autoLootItemIds.size() >= limit) {
    if (!isLogin)
      sendTextMessage(MESSAGE_STATUS_WARNING,
                      "You have reached the limit of items in the list.");
    return;
  }
...
bool Player::hasPvpActivity(Player *player,
                            bool guildAndParty /* = false*/) const {
  if (!g_game.isExpertPvpEnabled() || !player ||
      player == const_cast<Player *>(this)) {
    return false;
  }
...
void Player::sendPvpSquare(Creature *target, SquareColor_t squareColor) {
  ...
  if (squareColor == SQ_COLOR_YELLOW) {
    sendCreatureSquare(this, squareColor,
                       2); // Only add to self if it's yellow.
  }
}
